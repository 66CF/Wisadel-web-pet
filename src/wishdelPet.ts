...
function resolveAssetBase() {
  ...
  if (customBase) {
    return new URL(
      customBase.endsWith("/") ? customBase : `${customBase}/`,
      document.baseURI
    );
  }
...
declare global {
  interface Window {
    __wishdelPetAssetBase?: string;
    __wishdelPetController?: PetController;
  }
}
...
function parseStoredPoint(key: string): Point | null {
  try {
...
function createPetController(): PetController {
  ...
  const petShell = shadow.querySelector<HTMLElement>(".pet-shell");
  const petStage = shadow.querySelector<HTMLButtonElement>(".pet-stage");
  const petVideo = shadow.querySelector<HTMLVideoElement>(".pet-video");
  const petBubble = shadow.querySelector<HTMLElement>(".pet-bubble");
  ...
  const randomActionPool: AnimationName[] = [
    "interact",
    "relax",
    "attackA",
    "attackB",
  ...
  const chaseLines = ["朝你那边去。", "追上你。", "马上到。"] as const;

  const state = {
    current: "idle" as AnimationName,
    busy: false,
    dragging: false,
    roaming: false,
    visible: false,
    mode: "roam" as PetMode,
    pointerId: null as number | null,
    pointerDown: null as PointerState | null,
    clickTimer: null as number | null,
    idleTimer: null as number | null,
    messageTimer: null as number | null,
  ...
  function setFacing(direction: FacingDirection) {
    state.facing = direction;
    updateAnimationState();
  }

  function setBubble(text: string, sticky = false) {
  ...
  function getAnimationSource(file: string) {
  ...
  function getPetTargetFromCursor(clientX: number, clientY: number): Point {
    const width = petShellNode.offsetWidth || PET_SIZE;
    const height = petShellNode.offsetHeight || PET_SIZE;
  ...
  function faceCursor(clientX: number) {
    const width = petShellNode.offsetWidth || PET_SIZE;
  ...
  function beginMovement(nextX: number, nextY: number, line: string) {
    const movingDirection: FacingDirection =
      nextX >= state.renderX ? "right" : "left";
  ...
  function followCursor(clientX: number, clientY: number) {
  ...
  function playOneShot(name: AnimationName) {
  ...
  function finishCurrentAnimation() {
    ...
    const followUp =
      "followUp" in currentAnimation ? currentAnimation.followUp : "idle";
    state.busy = false;
  ...
  function updateDragPosition(clientX: number, clientY: number) {
    ...
    const next = clampPosition(
      clientX - state.pointerDown.x,
      clientY - state.pointerDown.y
    );
  ...
  function onClick() {
    ...
    state.clickTimer = window.setTimeout(() => {
      const quickAction =
        Math.random() > 0.55
          ? "interact"
          : pickRandom(["relax", "attackA", "attackB", "attackC"] as const);
  ...
  function syncButtons() {
    ...
    if (button) {
      ...
      const label = button.querySelector<HTMLElement>("[data-pet-label]");
      if (label) {
    ...
    const modeLabel = modeButton.querySelector<HTMLElement>("[data-pet-mode-label]");
    if (modeLabel) {
  ...
  function tick(timestamp: number) {
    ...
    const deltaSeconds =
      Math.min(0.05, (timestamp - state.lastFrameAt) / 1000 || 0.016);
    state.lastFrameAt = timestamp;
  ...
  function boot() {
    ...
    const initial = clampPosition(
      saved?.x ?? defaultPosition.x,
      saved?.y ?? defaultPosition.y
    );
